...
OAuth2.0 授权码授权流程如下，参考图：
1a. 访问请求
用户通过客户端访问微博。
例如：
    GET /weibo?user=jowin HTTP/1.1
    Host: weiboclient.com

1b. 重定向
客户端把用户导向认证服务器。
例如：
    HTTP/1.1 302 Found
    Location: https://authorize?response_type=code&client_id=s6BhdRkqt3&state=xyz&redirect_uri=https://weiboclient.com/cb
...
例如：
    GET /authorize?response_type=code&client_id=s6BhdRkqt3&state=xyz&redirect_uri=https://weiboclient.com/cb HTTP/1.1
    Host: weibo.com
...
例如：
...

    {
      ...
      "bearer_parameter":"example_value"
    }
...
参考：
  ...
  oauth2.0授权码模式详解    http://www.ruanyifeng.com/blog/2019/04/oauth-grant-types.html
